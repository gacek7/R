...
library("reshape2") #potrzebne do meltowania danych do heatmapy, aby zobaczyæ korelacje
...
colSums(is.na(data)) #tutaj sprawdzamy czy s¹ jakieœ braki w danych
#tutaj opisaæ metode analizy, czemu tak a nie inaczeji
...
#zmniejszamy skale dla obra¿en oraz zdobytego z³ota, 1 = 1 tysi¹c, bêdzie ³adniej na wykresach :)
...
#zabite miniony bêd¹ w dziesi¹tkach
...
#tutaj œrednie wszystkich zmiennych dla ka¿dego wariantu i dru¿yny
...
#dla wygody zmienimy nazwy ze œrednimi wynikami na 0, dopiszemy je potem aby by³o czytelniej
names(BWmean) <- NULL
names(BLmean) <- NULL
names(RWmean) <- NULL
names(RLmean) <- NULL
#Raczej koniec przygotowywania danych tutaj, jak coœ dopiszê to przepraszam ale z czasem coraz wiêcej pomys³ów :)
#Sprawdzmy ile przeciêtnie trwa rozgrywka
...
nrow(data[data$gameDuration<240,])/nrow(data) #procent skoñczonych gier przed 4 minut¹
(nrow(data[data$gameDuration<1000,])-nrow(data[data$gameDuration<900,]))/nrow(data) #procent gier skoñczonych przez early surr vote
#Wygrana zale¿na od strony po której sie zacze³o 
...
#nasze œrednie wyniki zamieniamy w dataframe i dopisujemy nazwy zmiennych
...
#œrednie wyniki gdy blue wygrywa a red przegrywa
meanBluevsRed <- data.frame(id=1:19,team=rep(c("Blue","Red"),each=19),Win=c(BWmean,RLmean),Lose=c(RWmean,BLmean),Variable=c("Win","First Blood",
                                                                                                                            "First Tower","First Baron","First Dragon","First inhibitor","Dragon Kills","Baron Kills","Tower Kills","Inhibitor Kills",
                                                                                                                            "WardPlaced","WardKills","Kills","Deaths","Asissts","Damage Dealt","Total Gold","Total minions","Average level"))
#œrednie wyniki gdy red wygrywa a blue przegrywa
...
#obie dru¿yny zwyciêskie przedstawione na wykresie, porównanie statystyk
...
plotLoseWin <- ggplot(meanRedvsBlue,aes(x = id,y = Win,fill = factor(team))) +
  ...
  theme(legend.position = c(.92,.75))
plotLoseWin
#korelacje dla wygranej niebieskiej, funkcja melt zamienia kolumne w pojedyñcz¹ wartoœæ
...
ggheatmap <- ggplot(melted_cormat, aes(Var2, Var1, fill = value))+
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, 
                                   size = 12, hjust = 1))+
  coord_fixed()
#edytuje macierz korelacji aby dostaæ ³adnie zedytowane wyniki
...
#wszystkie macierze korelacji wygl¹daj¹ bardzo podobnie, bliskie 1 wystêpuj¹ w tych samych miejscach